const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5043/api';
...
export async function getProducts(filters?: ProductFilters): Promise<ProductsResponse['data']> {
  try {
    ...
    console.log('✅ Products fetched:', result.data.products.length, 'of', result.data.totalCount);
...
export async function getProductById(id: number): Promise<Product> {
  try {
    ...
    console.log('✅ Product detail fetched:', result.data.title);
